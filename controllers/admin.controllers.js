...
adminController.getSingleUser = async (req, res) => {
  let user;
  try {
...
adminController.loginUser = async (req, res) => {
    try {
        const body = req.body;
        ...
        if (!result) {
          ...
          if ( bcrypt.compareSync(body.password, result.password)) {
            ...
            const token = jsonwebtoken.sign({
               data: result,
               role: 'User'
            }, process.env.JWT_KEYA, { expiresIn: '7d' });
...
adminController.getNextId = async (req, res) => {
  try {
...
adminController.uploadAvatar = async (req, res) => {
  try {
